...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    // Validate selections
    const specialityValidation = validateSelection(
      speciality,
      "votre spécialité",
    );
    if (!specialityValidation.isValid) {
      setError(specialityValidation.error);
      return;
    }
    ...
    if (speciality === "Pharmacie" || speciality === "Dentaire") {
      setError(
        "Cette spécialité sera bientôt disponible. Veuillez sélectionner Médecine pour le moment.",
      );
      return;
    }

    const yearValidation = validateSelection(
      yearOfStudy,
      "votre année d'étude",
    );
    ...
    if (speciality === "Médecine" && yearOfStudy !== "2") {
      setError(
        "Seule la 2ème année de Médecine est actuellement disponible. Les autres années seront bientôt disponibles.",
      );
      return;
    }

    const facultyValidation = validateSelection(
      faculty,
      "votre faculté / annexe",
    );
    if (!facultyValidation.isValid) {
      setError(facultyValidation.error);
      return;
    }
    ...
    const { error: registerError, needsEmailVerification } =
      await signUp(formData);
    if (registerError) {
      setError(registerError);
    } else {
      // Always show email verification screen after successful registration
      // Never auto-login - user must verify email and login manually
        ...
        >
          ...
          >
            ...
            <View style={{ gap: 16, zIndex: 300 }}>
              ...
              <View
                style={
                  isDesktop ? { flexDirection: "row", gap: 16 } : { gap: 16 }
                }
              >
              ...
              >
                <View style={[isDesktop ? { flex: 1 } : {}, { zIndex: 20 }]}>
                  ...
                  <FormDropdown
                    ...
                    options={SPECIALITIES.map((s) => ({
                      value: s.value,
                      label:
                        s.value === "Pharmacie" || s.value === "Dentaire"
                ...
                <View style={[isDesktop ? { flex: 1 } : {}, { zIndex: 10 }]}>
                  ...
                  <FormDropdown
                    value={
                      yearOfStudy
                        ? YEARS.find((y) => y.value === yearOfStudy)?.label ||
                          ""
                        : ""
                    }
                    placeholder={
              ...
              >
                <View style={[isDesktop ? { flex: 1 } : {}, { zIndex: 20 }]}>
                  ...
                  <FormDropdown
                    value={
                      faculty
                        ? FACULTIES.find((f) => f.value === faculty)?.label ||
                          ""
                        : ""
                    }
                    placeholder="Sélectionner"
            ...
            >
              ...
              <TouchableOpacity onPress={() => router.push("/(auth)/login")}>
                <Text
                  style={{ color: "#09B2AD", fontWeight: "700", fontSize: 15 }}
                >
                  Se connecter
                </Text>
              </TouchableOpacity>
    ...
    <>
      ...
      >
        <Text
          style={{
            color: value
              ? BRAND_THEME.colors.gray[900]
              : BRAND_THEME.colors.gray[400],
            fontSize: 16,
          }}
        >
          {value || placeholder}
        </Text>
        <Text style={{ color: BRAND_THEME.colors.gray[400] }}>▼</Text>
      </TouchableOpacity>
          ...
          >
            ...
            >
              ...
              <TouchableOpacity onPress={onToggle} style={{ padding: 4 }}>
                <Text
                  style={{ fontSize: 20, color: BRAND_THEME.colors.gray[400] }}
                >
                  ✕
                </Text>
              </TouchableOpacity>
              ...
              renderItem={({ item: opt }) => (
                <TouchableOpacity
                  style={{
                    ...
                    backgroundColor:
                      value === opt.value ||
                      (value === opt.label && !opt.label.includes("cod"))
                        ? "rgba(9, 178, 173, 0.05)"
                        : "transparent",
                  }}
                  onPress={() => {
