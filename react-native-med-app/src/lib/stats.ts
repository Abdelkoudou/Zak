...
import { OfflineStatsService } from './offline-stats'
...
export async function saveTestAttempt(data: TestAttemptData): Promise<{
  ...
  try {
    ...
    console.log('[Stats] Saving attempt offline due to error:', error)

    // Save to queue
    const id = await OfflineStatsService.savePendingAttempt(data)

    if (id) {
      // Return a mock attempt so UI proceeds "optimistically"
      ...
      return { attempt: mockAttempt, error: null }
    }

    return { attempt: null, error: 'Failed to save test attempt (offline & online failed)' }
  }
}
...
export async function getUserStatistics(userId: string): Promise<{
  ...
  try {
    ...
    if (!attemptsError) {
      ...
      if (!savedError) {
        // Calculate statistics locally from fresh data
        ...
        // 2. Cache the fresh stats
        await OfflineStatsService.saveCachedStats(stats)
      }
    }

    // 3. Return the Aggregated Stats (Cached + Pending)
    // This handles both cases:
    // - If online fetch succeeded, we just updated the cache, so getAggregatedStats returns Fresh + Pending.
    // - If online fetch failed, we assume cache is untouched, so getUserStatistics returns Old Cache + Pending.
    // This ensures "Optimistic" Updates are always visible immediately after saving locally.

    const aggregatedStats = await OfflineStatsService.getAggregatedStats()
    return { stats: aggregatedStats, error: null }

  } catch (error) {
    // If everything fails, try one last time to get whatever we have locally
    const fallback = await OfflineStatsService.getAggregatedStats()
    return { stats: fallback, error: null }
  }
}
