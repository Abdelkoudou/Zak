...
export default function UsersPage() {
  ...
  const getSubscriptionStatus = (user: ManagedUser) => {
    ...
    if (!user.subscriptionExpiresAt) return { label: 'Payé', color: 'bg-green-500' };
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl">💳</span>
                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Payés</span>
              </div>
              <p className="text-3xl font-black text-green-500">{stats.paidUsers}</p>
            </div>

            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl">✅</span>
                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Actifs</span>
              </div>
              <p className="text-3xl font-black text-primary-500">{stats.activeSubscriptions}</p>
            </div>

            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl">⏰</span>
                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Expirés</span>
              </div>
              <p className="text-3xl font-black text-red-500">{stats.expiredUsers}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4">
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Non activés</p>
              <p className="text-2xl font-black text-slate-900 dark:text-white">{stats.freeUsers}</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4">
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Appareils</p>
              <p className="text-2xl font-black text-slate-900 dark:text-white">{stats.totalDevices}</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4">
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">1ère Année</p>
              <p className="text-2xl font-black text-slate-900 dark:text-white">{stats.year1Users}</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4">
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">2ème Année</p>
              <p className="text-2xl font-black text-slate-900 dark:text-white">{stats.year2Users}</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {[
                { year: '1ère année', count: stats.year1Users, color: 'bg-blue-500' },
                { year: '2ème année', count: stats.year2Users, color: 'bg-green-500' },
                { year: '3ème année', count: stats.year3Users, color: 'bg-purple-500' },
              ].map(item => (
                <div key={item.year} className="flex items-center gap-4">
                  <span className="text-sm text-slate-600 dark:text-slate-400 w-24">{item.year}</span>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4">
            <div className="flex flex-wrap gap-3">
              ...
              <select
                ...
                onChange={(e) => setFilters({ ...filters, isPaid: e.target.value as typeof filters.isPaid })}
              ...
              <select
                ...
                onChange={(e) => setFilters({ ...filters, yearOfStudy: e.target.value as YearLevel | '' })}
                className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm"
              >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden">
            {fetching ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="font-medium text-slate-900 dark:text-white mb-1">Aucun utilisateur trouvé</p>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className="text-sm text-slate-700 dark:text-slate-300">
                              {user.yearOfStudy ? `${user.yearOfStudy}ère` : '-'}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`text-[10px] px-2 py-1 ${status.color} text-white font-bold rounded-md uppercase`}>
                              {status.label}
                            </span>
                          ...
                          <td className="px-4 py-3">
                            <button
                              onClick={() => setDeviceManagerUser({ id: user.id, name: user.fullName || user.email })}
